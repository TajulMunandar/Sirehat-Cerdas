    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isEditMode && currentItemId) {
                await router.put(`/dashboard/apoteker/${currentItemId}`, data, {
                    onSuccess: (data: any) => {
                        console.log(data);
                ...
                await router.post(`/dashboard/apoteker`, data, {
                    onSuccess: (data: any) => {
                        console.log(data);
                        if (data.props.status_code == 500) {
                            toast.error(
                                "Error Add apoteker, Username Already Taken"
                            );
                        } else {
                            toast.success("Apoteker add successfully");
                        }
                        setIsDeleteConfirmationOpen(false);
                        closeModal();
                    },
    ...
    const handleConfirmDelete = async () => {
        try {
            if (deleteItemId !== null) {
                await router.delete(`/dashboard/apoteker/${deleteItemId}`, {
                    onSuccess: (data: any) => {
                        console.log(data);
                        if (data.props.status_code == 500) {
                            toast.error("Error deleting apoteker");
                        } else {
